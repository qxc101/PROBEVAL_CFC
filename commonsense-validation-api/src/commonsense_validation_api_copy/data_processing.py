...
__all__ = [
    ...
    "load_jsonl_to_list",
    "save_list_to_jsonl",
    "load_question_answer_clusters_from_jsonl_file",
    "load_predictions_from_jsonl_file"
]
...
def load_question_answer_clusters_from_jsonl(q_json) -> Dict[str, QuestionAndAnswerClusters]:
    """
    Load jsonl answer cluster data for evaluation.
    Attempts to handle multiple formats of input jsonl files.

    :param q_json: json data for a question
    :return: Dict from question_id to QuestionAndAnswerClusters
    """
    question_data = dict()
    question = q_json["question"]
    question_id = q_json["metadata"]
    answer_clusters = {frozenset(answer_cluster["answers"]): [answer_cluster["count"], k]
            for k, answer_cluster in (q_json["answers"]["clusters"].items())}
    raw_answers = q_json["answers"]["raw_answers"]  
...
def load_question_answer_clusters_from_jsonl_file(
        data_path: Union[Path, str]
    ...
    with open(data_path) as data:
        for q in data:
            ...
            if "answers" in q_json and "clusters" in q_json["answers"]:
                q_json["answers"]["clusters"]["wrong"] = {}
                q_json["answers"]["clusters"]["wrong"]["answers"] = []
                q_json["answers"]["clusters"]["wrong"]["count"] = 0
                answer_clusters = {
                    frozenset(answer_cluster["answers"]): [answer_cluster["count"], k]
                    for k, answer_cluster in (q_json["answers"]["clusters"].items())
                }
                raw_answers = q_json["answers"]["raw"]
...
def load_predictions_from_jsonl(data_path) -> Dict[str, List[str]]:
    """
    ...
    ans_dict = dict()
    try:
        qid, ans = convert_JSONL_line_to_prediction(data_path)
        ans_dict[qid] = ans
...
def load_predictions_from_jsonl_file(data_path: Union[Path, str]) -> Dict[str, List[str]]:
    """
    Loads JSONL into a simplified dictionary structure which only maps qids to lists of answers.
    If data_path is a JSON file, it should be in this format:
    ```
