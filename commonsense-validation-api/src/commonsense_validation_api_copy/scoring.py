...
def all_pairs_scores(
    ...
    for (a_idx, a_val), (b_idx, b_val) in product(enumerate(a), enumerate(b)):
        ...
        if not (0 <= score_val <= 1):
            print(f"Score function did not return a value in [0,1]: ", f"score_func({a_val}, {b_val}) = {score_val} with type {type(score_val)}")
            warnings.warn(
                f"Score function did not return a value in [0,1]: "
                f"score_func({a_val}, {b_val}) = {score_val} with type {type(score_val)}"
            )
    t = reduction_func(score_matrix)
    # print()
    # print(time.time() - s)
...
def wordnet_score(
    ...
    *,
    ...
    def _preprocess(z, stopwords=stopwords, max_length=5):
...
def cluster_score(
    ...
    # import pdb; pdb.set_trace()
    t = all_pairs_scores(pred_answers, true_answers, score_func)
    return t
